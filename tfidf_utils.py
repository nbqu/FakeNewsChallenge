...
def pipeline_train(train, test, lim_unigram):
    ...
    return x, train_stances, bow_vectorizer, tfreq_vectorizer, tfidf_vectorizer
...
def save_predictions(pred, origin, file):
    ...
    with open(file, 'w') as csvfile:
        ...
        writer.writeheader()
        for instance, origin_dict in zip(pred, origin.instances):
...
class FNCDataset(Dataset):
    ...
    def __getitem__(self, index):
        return self.x[index], self.y[index] if self.y else self.x[index]
